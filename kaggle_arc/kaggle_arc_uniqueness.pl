...
%:- decl_pt(prop_g,point_groups(is_grid,key,term)).
point_groups(Grid,Key,StructureLL):-
  segs_of(Grid, _SegOptions, _ObjsCount, PointsList),
  %Points = Structure,
  maplist(add_lengths,PointsList,LenList,Lens),
  sort(LenList,SLenList),
  sort(Lens,SLens),
  points_to_structure(SLenList,StructureL), !,
  member(Key,SLens),
  findall(Structure,member(Key-Structure,StructureL),StructureLL).
add_lengths(Ps,Key-Ps,Key):- length(Ps,Key).

points_to_structure(SLenList,StructureL):- nth1(4,SLenList,N-_),!, maplist(points_to_structure_g(N),SLenList,StructureL).
points_to_structure(SLenList,StructureL):- maplist(points_to_structure_g(0),SLenList,StructureL).

points_to_structure_g(1,1-Points,1-gps(Points)):-!.
points_to_structure_g(0,1-Points,1-gps(Points)):-!.
points_to_structure_g(_,N-GGPoints,N-Structure):-
 globalpoints(GGPoints,GPoints),
 physical_points(GPoints,PGPoints),
 sort_points(PGPoints,PointsL),
  %po_loc2D_vis2D(PointsL,[],LocX,LocY,SizeX,SizeY),
 gpoints_to_iv_info(PointsL,_ShapePoints,_LocX,_LocY,_PenColors,_RotG,_Iv,[],LPoints0,_Grid,_OX,_OY,_SizeX,_SizeY,_CentX,_CentY),
 %make_localpoints(ShapePoints,RotG,OX,OY,PenColors,_CheckLPoints),
 Structure = lps(LPoints0).
...
((
  ...
  *-> true ; (fail,show_last_chance(perfect_copy(Min..Max, Nth),LHSObjs,RHSObjs),!,fail)),
  PCRule = rule(InfoInOut, perfect_copy(Min..Max, Max), LHS), % Maybe use Nth
...
% Scenery
calc_o_d_recursively(_VM, ActionGroupIn, Info, PrevRules, LHSObjs, RHSObjs,
                ActionGroupOut, InfoOut, RulesOut, LHSObjs, RHSObjsRest):-  fail_ads,
...
find_relative_sr(_VM,Same, Info,_IDNth_, Left, Right, PrevRules, R, NewRSet, PreCond, XtraRule):- !,
  ...
  ((member(exists_as(left,P,perfect_copy),PrevRules),is_object(P),fail_if_first_try);(fail,P=Left)),
...
%indv_has_conds_1(_Rules,_VM,_Obj, P, H):- \+ \+ secretly_allow(P,H),!.
...
secretly_allow(P,_):- functor(P,perfect_copy,_),!,fail.
secretly_allow(_,sid(_)).
secretly_allow(_,iz(cenX(_))).
secretly_allow(_,iz((_))).
secretly_allow(_,grid_ops(_,_)).
secretly_allow(_,sym_counts(_,_)).
secretly_allow(_,pg(_,_,_,_)).
...
apply_obj_transforms(VM, TestID, ExampleNum, Ctx, Rules, Objs, ObjsO):-
  fail, apply_copy_perfects(VM, TestID, ExampleNum, Ctx, Rules, Objs, Used, MidNewObjs, Uncopied), Used\==[], !,
...
apply_obj_transforms(_VM,_TestID,_ExampleNum,_Ctx, Rules,Obj, []):-  member(Rule,Rules),rule_head(Rule,Head), is_functor(delete_object,Head), rule_body(Rule,Body), member(P,Body),has_prop(P,Obj),!.
...
apply_obj_transforms(VM, _TestID, _ExampleNum, _Ctx, Rules, Obj, [NewObj]):-
 must_det_ll((
  pp_ilp(skipped_obj=Obj),
  indv_props_list(Obj,Props),sort(Props,SL),include_not_already_true(SL,DoesHave),
  pp_ilp(does_have=DoesHave),
  maplist(obj_violations(DoesHave),Rules,Sortable),
  sort(Sortable,Sorted),
  pp_ilp(sorted=Sorted),
  nth1(1,Sorted,Best),
  %functor(Best,_,A),
  arg(1,Best,P), arg(2,Best,PConds),
  %pp_ilp(best=Best),
  apply_rule(VM, Obj, P, PConds, NewObj))).
 %nop(override_object_1(VM, pen([cc(brown, 1)]), NewObj1, NewObj)))), !.
%apply_obj_transforms(_VM, _TestID, _ExampleNum, _Ctx, _Rules, Objs, Objs).


obj_violations(DoesHave,Rule,( (Sortable):-[already(AlreadyHas),missing(WhyDoesnt)])):-
  rule_units(Rule,_Ctx, P, Conds),
  include_not_already_true(Conds,DoesNeed),
  intersection(DoesHave,DoesNeed,AlreadyHas,_Extra,WhyDoesnt),!,
  score_of_has(AlreadyHas,AHScore), score_of_hasnt(WhyDoesnt,WDScore),
  head_penalty(P,Penal),
  Sortable = sortable(Penal,WDScore,AHScore,P).

score_of_has([],inf).
score_of_has(AlreadyHas,AHScore):- length(AlreadyHas,AHL),AHScore is - AHL.

score_of_hasnt([],0).
score_of_hasnt(WhyDoesnt,WDScore):- length(WhyDoesnt,WDScore).

head_penalty(P,Penal):- compound(P),functor(P,F,_),head_penalty(F,Penal),!.
%head_penalty(perfect_copy,2).
%head_penalty(copy_with_edit,0).
head_penalty(of_obj_use,inf).
head_penalty(perfect_copy1,inf).
head_penalty(_,1).
...
try_one_rule(VM, _TestID, _ExampleNum, Ctx, Rules, Obj, NewObj):-
 ...
 % P \= perfect_copy(_,_),
  indv_has_conds(Rules, VM, Obj, P, PConds),!,
  must_det_ll(apply_rule(VM, Obj, P, PConds, NewObj)).
/*
try_one_rule(VM, _TestID, _ExampleNum, Ctx, Rules, Obj, NewObj):-
  rule_copies_no_happy_endings(Rules,Ctx,P,PConds),%member(M,PConds),
  \+ is_task_rule_head(P),
...
apply_rule(VM, Obj, P, Info, NewObj):-
