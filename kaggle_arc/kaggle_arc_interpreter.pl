...
:- writeln(missing(ensure_loaded(library(logicmoo/typesystem/mpred_type_constraints)))).
...
should_not_enum_into_grid:-!.

:- decl_pt(into_grid(+(prefer_grid),-grid)).
into_grid(P,_):- var(P), should_not_enum_into_grid, !, fail.
into_grid(P,G):- is_grid(G),var(P),P=G,!.
...
cast_to_grid(P,G, =):- var(P),should_not_enum_into_grid,!,P=G.
...
if_or_else(G1,G2):- call(G1)*->true;call(G2).

grid_to_gid(Grid,GID):- if_or_else(grid_to_gid1(Grid,GID),if_or_else(grid_to_gid2(Grid,GID),grid_to_gid3(Grid,GID))).
grid_to_gid1(Grid,GID):- was_grid_gid(Grid,GID).
grid_to_gid2(Grid,GID):- grid_to_etid(Grid,ID),ensure_now_tid_gids(Grid,ID,GID).
grid_to_gid3(Grid,GID):- grid_to_tid(Grid,ID),ensure_now_tid_gids(Grid,ID,GID).
...
oid_to_gridoid(GID,G):- current_predicate(gid_to_grid/2),
  call(call,gid_to_grid,GID,G).
oid_to_gridoid(ID,G):-  atom(ID),atomic_list_concat(Term,'_',ID),
  append([o,_,_],GOID,Term),
