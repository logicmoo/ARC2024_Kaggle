...
ensure_gid(Grid,GID):- grid_to_gid(Grid,GID),once(assert_id_grid_cells(GID,Grid)).
...
assert_grid_cmem(GID,SH,SV,Grid):- var(Grid),!,
    ignore((ground(SH+SV),assert_if_new(is_grid_size(GID,SH,SV)))).
