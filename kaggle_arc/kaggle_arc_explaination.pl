...
print_individuals(TestID,ExampleNum):- \+ ground(ExampleNum),!,
  is_arc_task(TestID,ExampleNum),ground(ExampleNum),print_individuals(TestID,ExampleNum).
print_individuals(TestID,ExampleNum):-
  is_arc_task(TestID,ExampleNum),
 ((
         %     gid_of_tid(GID1,TestID,ExampleNum,in),
         %     gid_of_tid(GID2,TestID,ExampleNum,out),
   ignore(ensure_individuals(TestID,ExampleNum)),
   findall(wqs([Len,ROptions,GID1])=In,
     (is_arc_task(TestID,ExampleNum),arc_cache:individuated_cache(TestID,TestID>ExampleNum*in,GID1,ROptions,In),
      length(In,Len)),InL),sort(InL,InS),
   findall(wqs([Len,ROptions,GID2])=Out,
      (is_arc_task(TestID,ExampleNum),arc_cache:individuated_cache(TestID,TestID>ExampleNum*out,GID2,ROptions,Out),
