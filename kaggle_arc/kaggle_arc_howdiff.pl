...
grid_props(Obj1,OOO):- % \+ arc_option(grid_size_only), 
 ...
 findall(Prop, ((
  ...
  P1P2=..[P2,P1|TypeList], once((on_x_log_and_fail(is_type_call(P1,Obj1)))),
    length(TypeList,Len), length(NewLst1,Len), 
     once_if_not_key(TypeList,(on_x_log_and_fail(apply(P2,[Obj1|NewLst1])))),
     Prop =.. [P2|NewLst1])), 
   ListO),
   %OOO = [toStr(S)|ListO],
   OOO = ListO,
...
once_if_not_key(TypeList,Goal):- \+ sub_var(key,TypeList),!,once(Goal).
once_if_not_key(_TypeList,Goal):- call(Goal).
...
%:- decl_pt(prop_g,grid_get_value(prefer_grid,name,set)).
