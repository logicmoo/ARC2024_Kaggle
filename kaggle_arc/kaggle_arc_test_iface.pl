...
menu_cmd1(_,'j','                  or (j)unctions between objects',   (cls_z_make,!,
     locally(nb_setval(show_indiv,f),ndividuator),show_object_dependancy)).
...
report_suite_test_count:- luser_getval(test_suite_name,SuiteX), write('\n--->>>>'),report_suite_test_count(false,SuiteX).

report_suite_test_count(EnsureCalc,SuiteX):-
  color_print(yellow,SuiteX),color_print(yellow,"="),
  (EnsureCalc -> current_suite_testnames(SuiteX,_); true),
  ((muarc_tmp:cached_tests(SuiteX,Set),length(Set,Len)) -> true ; Len = ?),
  print_suite_link(SuiteX,Len).

print_suite_link(passed(_SuiteX),Len):-  bold_print(underline_print(color_print(green,passed(Len)))).
print_suite_link(failed(_SuiteX),Len):-  bold_print(underline_print(color_print(red,failed(Len)))).
print_suite_link(_SuiteX,Len):-  bold_print(underline_print(color_print(yellow,Len))).
...
into_test_id_list(TestList,Tests):-  var(TestList),!,Tests=[].
into_test_id_list(TestList,Tests):-  TestList==[], !,Tests=[].
into_test_id_list(TestList,Tests):- \+ atom(TestList), catch(text_to_atom(TestList,Atom),_,fail),!,into_test_id_list(Atom,Tests).
...
into_test_id_list(String,Tests):- split_string(String,",[] \n\r\t\s",",[] \n\r\t\s",L),
   L\==[String], !,maplist(into_test_id_list,L,TestsL),append(TestsL,Tests).
...
is_test_pre_normalizable(T,IO):- var(T),!,is_arc_task(T),is_test_pre_normalizable(T,IO).
 ...
 when_html(write('<style type="text/css">a {color: cyan;} body { background-blend-mode: difference; mix-blend-mode: difference}</style>')),
  ((forall(nth_above(300,N,L,SuiteX),
   (%nl,%current_suite_testnames(SuiteX,_),
    nl,print_menu_cmd1((format(' ~w:  ',[N]),
    report_suite_test_count(false,SuiteX)),N),ignore((SuiteX==SuiteXC,write('   <<<<-------'))))))).
...
report_suites:-
 ...
 when_html(write('<style type="text/css">a {color: cyan;} body { background-blend-mode: difference; mix-blend-mode: difference}</style>')),
  ((forall(nth_above(300,N,L,SuiteX),
   (%nl,%current_suite_testnames(SuiteX,_),
    nl,/*print_menu_cmd1*/((format(' ~w:  ',[N]),
    report_suite_test_count(true,SuiteX))),ignore((SuiteX==SuiteXC,write('   <<<<-------'))))))).

/*
 ((forall(nth_above(300,N,L,SuiteX),
  (nl,wots(SS,(format(' ~w:  ',[N]),report_suite_test_count(true,SuiteX))),
...
is_valid_testname(TestID):- compound(TestID), is_arc_task(TestID),!.
is_valid_atom_testname(TestID):- atom(TestID), once(is_arc_task(t(TestID));is_arc_task(v(TestID))).
...
first_current_example_num(TrnN):- some_current_example_num(TrnN),ground(TrnN),TrnN\==[],
  get_current_test(TestID),is_arc_task(TestID,TrnN),!.
...
pair_count(Trn,C):- get_current_test(TestID),findall(N2,is_arc_task(TestID,Trn+N2),List),sort_safe(List,Sort),last(Sort,C).
...
all_arc_test_name_unordered(TestID):- is_arc_task(TestID).
...
%some_test_info_1(TestID,EEE):- var(TestID),!,is_arc_task(TestID),some_test_info_1(TestID,EEE).
...
test_names_ord_hard(NamesByHard):-
  ...
  pp(done_recreating(test_names_ord_hard)),
...
hardness_of_name(TestID,Hard):-
 %ExampleNum=tst+_,
 ExampleNum=_,
 findall(_,kag gle_arc(TestID,(trn+_),_,_),Trns),
 length(Trns,TrnsL),
...
testid_name_num_io(ID,ID,_Example,_Num,_IO):- compound(ID), \+ \+ is_arc_task(ID),!.
testid_name_num_io(ID,TestID,Example,Num,IO):-
  ...
  ignore((fail, Modes=[+,-|_], nonvar(TestID), is_arc_task(TestID), really_set_current_test(TestID))).
...
fix_id_1(Tried,   Tried):- is_arc_task(Tried),!.
...
atom_id_e(Tried,t(Tried)):- is_arc_task(t(Tried)),!.
atom_id_e(Tried,v(Tried)):- is_arc_task(v(Tried)),!.
atom_id_e(Tried,x(Tried)):- is_arc_task(x(Tried)),!.
