...
:- http_handler('/', swish_arc_root, [prefix,priority(300)]).
...
start_arc_http_server(Port) :-
   ...
   % catch_log(http_server(http_dispatch, [port(Port),ip('10.0.0.122'),workers(2)])),
    catch_log(http_server(http_dispatch, [port(Port),ip('0.0.0.0'),workers(10)])),
    !.
...
<!-- DataTables JS -->
<script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.11.5/js/jquery.dataTables.min.js" crossorigin="anonymous"></script>
...
% ansi
arcproc_now_main :- set_current_arc_cmds, call_current_arc_cmd(webcmd),!.
...
set_current_arc_cmds:- get_now_cmd('task',TaskID),set_current_test(TaskID),luser_setval('webcmd',TaskID),fail.
set_current_arc_cmds:- !.

call_current_arc_cmds:- set_current_arc_cmds, luser_getval('webcmd',Prolog), dmsg(call_current_arc_cmds(webcmd)=Prolog), !,invoke_arc_cmd(Prolog).
