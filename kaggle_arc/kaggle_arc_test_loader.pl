...
load_json_files(F, FName):- is_list(FName),!,my_maplist(load_json_files(F), FName).
load_json_files(F, File):- atomic(File), exists_file(File), !, load_arc_test_file(F,File).
...
%load_json_files(F, FName):- exists_file(FName),\+ is_absolute_file_name(FName), absolute_file_name(FName,ABSName),FName\==ABSName,!,load_arc_test_file(F, ABSName).
load_json_files(F, FName):- load_arc_test_file(F, FName),!.


%load_arc_test_file(F, FName):- atomic(FName), \+ is_absolute_file_name(FName),
%  absolute_file_name(FName,ABSName),FName\=@=ABSName,!,load_arc_test_file(F, ABSName),!.

%load_arc_test_file(F, PathMasked):- (\+ atomic(PathMasked); \+ exists_file(PathMasked); atom_contains(PathMasked,'*')),
%  expand_json_files(F,PathMasked).

load_arc_test_file(_F,PathMasked):- atomic(PathMasked),
  \+ atom_concat(_,'.csv',PathMasked),
  \+ atom_concat(_,'.json',PathMasked),!,
  wdmsg(skipping_file(PathMasked)),!.

load_arc_test_file(F, FullName):-
  file_name_extension(_,Ext,FullName),
  load_arc_test_file(Ext,F, FullName).

load_arc_test_file(json, F, FullName):-
  %pp(load_arc_test_file(F)=FullName),!,
  must_det_ll((
  fullname_to_basename(FullName,Name),
  Testname=..[F,Name],
  ...
  locally(t_l:local_test_props(filename=FullName),
    (load_json_of_file(Testname,file,Term),
     ignore((
      is_arc_task(Testname),
      add_test_info(Testname),
      add_testfile_name(Testname,FullName))))))),!.

load_arc_test_file(csv, F, FullName):-
  %pp(load_arc_test_file(F)=FullName),!,
 must_det_ll((
  csv_read_file(FullName,CSV,[convert(false)]),
  maplist(load_csv_grid(F,FullName),CSV))).
...
load_csv_grid(_F,_FullName,row(output_id,_GridStr)):- !.
load_csv_grid(_F,_FullName,row(_,'')):- !.
load_csv_grid(F,FullName,row(TestID_Num,GridStr)):-
 must_det_ll((
  atomic_list_concat([TestAtom,ANum],'_',TestID_Num),
  Testname=..[F,TestAtom],
  atomic_list_concat(EachGridStr,' ',GridStr),
  atom_number(ANum,Num),
  add_testfile_name(Testname,FullName),
  fullname_to_basename(FullName,SuiteName),
  %add_test_info_prop(Testname,test_suite,SuiteName),
  atom_concat('fail_',SuiteName,FailSuite),
  add_test_info_prop(Testname,test_suite,FailSuite),
  asserta_if_new(dir_test_suite_name(FailSuite)),
  atom_concat('pass_',SuiteName,PassSuite),
  asserta_if_new(dir_test_suite_name(PassSuite)),
  %nl,print(FullName=row(TestID_Num,GridStr)),nl,
  load_csv_grid10(FullName,SuiteName,Testname,Num,1,EachGridStr),
  add_test_info(Testname))).

load_csv_grid10(_FullName,_SuiteName,_Testname,_Num,_Try,[]).
load_csv_grid10(FullName,SuiteName,Testname,Num,Try,[EachGridStr|More]):-!,
  load_csv_grid11(FullName,SuiteName,Testname,Num,Try,EachGridStr),
  TryPlus1 is Try+1, load_csv_grid10(FullName,SuiteName,Testname,Num,TryPlus1,More).

dumb_grid_string('|123|456|789|').
dumb_grid_string('|0|').
dumb_grid_string('').

load_csv_grid11(_FullName,_SuiteName,_Testname,_Num,_Try,Dumb):- dumb_grid_string(Dumb), !.
load_csv_grid11(FullName,SuiteName,Testname,Num,Try,EachGridStr):-
 must_det_ll((
  atomic_list_concat(EachRowIn,'|',EachGridStr),
  include(\==(''),EachRowIn,EachRow),
  maplist(row_to_color_list,EachRow,ColorRow),
  NumTryN is 100+(Num*10)+(Try-1),
  ExampleNum = (PassFailSuite+NumTryN),
  TestRef = (Testname>(tst+Num)*out),
  once(((is_kaggle_arc_io(Testname,(tst+Num),out,Expected),is_grid(Expected))-> true ; Expected=[[unkC]])),
  atom_concat('pass_',SuiteName,PassSuite),
  atom_concat('fail_',SuiteName,FailSuite),
   ((Expected=@=ColorRow) -> (add_test_info_prop(Testname,test_suite,PassSuite),PassFailSuite=PassSuite,
      rem_test_info_prop(Testname,test_suite,FailSuite)) ; PassFailSuite = FailSuite),

  (nb_current(print_csv_load,t)->
   print_ss(wqs(try(PassFailSuite,filename(FullName),num(Num),try(Try),numtry(ExampleNum),TestRef)),Expected,ColorRow); true),
  nop(assert_kaggle_arc_now(Testname,ExampleNum,Expected,ColorRow)))).

row_to_color_list(Atom112233,Colors112233):-
  atom_chars(Atom112233,Chars112233),
  maplist(atom_number,Chars112233,Ints112233),
  maplist(grid_color_code,Ints112233,Colors112233).
...
rem_test_info_prop(Name,F,V):-
  must_det_ll((
    NV=..[F,[]],ignore(retract(muarc_tmp:some_test_info_prop(Name,NV))),
   n_v_to_nv(F,[V],TV),
   retractall(muarc_tmp:some_test_info_prop(Name,TV)),
   nop(u_dmsg(muarc_tmp:some_test_info_prop(Name,TV))))).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
:- dynamic(is_arc_task/1).
%is_arc_task(TestID):- is_arc_task(TestID,trn+0).
%%%%%%%%%%%%%%%%%%%%%%%%%%
%:- dynamic(is_arc_task/2).
is_arc_task(TestID,ExampleNum):- is_kaggle_arc_io(TestID,ExampleNum,in,_).
%%%%%%%%%%%%%%%%%%%%%%%%%%

:- dynamic(is_kaggle_arc_io/4).
kaggle_arc_json(Name,Type,In,Out):-
  %if_t(var(Type),(bt,break)),
  %is_arc_task(Name),
  %is_arc_task(Name,Type),
  is_kaggle_arc_io(Name,Type,in,In),
  kaggle_arc_io(Name,Type,out,Out).

kaggle_arc_io(Name,Type,IO,In):- (is_kaggle_arc_io(Name,Type,IO,In)*->true;In=[[unkC]]).
/*
...
%assert_in(Term):- copy_term(Term,Copy), clause(Copy,true),Copy=@=Term,!.
assert_in(Term):- assert_if_new(Term).
...
/*
%assert_kaggle_arc_json_now(Name,Type,In,Out):- kaggle_arc(Name,Type,In,OutO),Out=@=OutO,!.
assert_kaggle_arc_json_now(Name,Type,In,Out):-  var(In), kaggle_arc_json(Name,Type,In1,_Out),is_grid(In1),!,
  assert_kaggle_arc_json_now(Name,Type,In1,Out).
assert_kaggle_arc_json_now(Name,Type,In,Out):-  var(Out), kaggle_arc_json(Name,Type,_In,Out1),is_grid(Out1),!,
  assert_kaggle_arc_json_now(Name,Type,In,Out1).
assert_kaggle_arc_json_now(Name,Type,In,Out):-  var(In),!,assert_kaggle_arc_json_now(Name,Type,[[unkC]],Out).
assert_kaggle_arc_json_now(Name,Type,In,Out):-  var(Out),!,assert_kaggle_arc_json_now(Name,Type,In,[[unkC]]).
*/
assert_kaggle_arc_json_now(Name,Type,In,Out):-
  assert_in(is_arc_task(Name)),
  %assert_in(is_arc_task(Name,Type)),
  into_solid_grid(In,InS),into_solid_grid(Out,OutS),!,
  if_t(is_good_grid(InS),assert_in(is_kaggle_arc_io(Name,Type,in,InS))),
  if_t(is_good_grid(OutS),assert_in(is_kaggle_arc_io(Name,Type,out,OutS))),
  nop(really_assert_kaggle_arc_json_now(Name,Type,InS,OutS)).

is_good_grid(InS):- InS\=[[unkC]].


really_assert_kaggle_arc_json_now(Name,Type,In,Out):-
 Term = kaggle_arc_json(Name,Type,In,Out),
 copy_term(Term,Copy), clause(Copy,true),Copy=@=Term,!.
really_assert_kaggle_arc_json_now(Name,Type,In,Out):- In\==[[black]],
 retract(kaggle_arc_json(Name,Type,[[black]],Out2)),
 really_assert_kaggle_arc_json_now(Name,Type,In,Out2),
 really_assert_kaggle_arc_json_now(Name,Type,In,Out).
really_assert_kaggle_arc_json_now(Name,Type,In,Out):- Out\==[[black]],
 retract(kaggle_arc_json(Name,Type,In2,[[black]])),
 really_assert_kaggle_arc_json_now(Name,Type,In2,Out),
 really_assert_kaggle_arc_json_now(Name,Type,In,Out).
really_assert_kaggle_arc_json_now(Name,Type,In,Out):-
 asserta(kaggle_arc_json(Name,Type,In,Out)),
 add_test_info(Name).
...
json_to_colors(Out,Color):- var(Out),!,Color=Out.
...
load_json_files1:- load_json_files(t,'../shared/data/ConceptARC/corpus/').
%% load_json_files1:- load_json_files(cvs_files,v,'../shared/data/arc-subs/?*.csv').
%% load_json_files1:- load_json_files(cvs_files,v,'../shared/data/arc-subs/?*.csv').

%%load_json_files1:- locally(nb_setval(print_csv_load,t),forall(load_csv_files1,true)).
...
load_csv_files1:- load_json_files(v,'../ARC-Others/arc_baseline/secret_data/evaluation/').
load_csv_files1:- load_json_files(v,'../ARC-Others/arc_baseline/secret_data/solution/').
%load_csv_files1:- load_json_files(v,'../ARC-Others/arc_baseline/secret_data/solution/*/*csv').
